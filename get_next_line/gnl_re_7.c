...
/*   Updated: 2021/12/13 16:56:22 by haryu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_str	*ft_new_list(t_str *lst, int fd, int code)
{
	t_str	*new;
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		...
		buff[read_cnt] = '\0';
		node->backup = ft_strnjoin(node->backup, buff, \
				(int)ft_strlen(node->backup) + BUFFER_SIZE);
		if (read_n_find(fd, node->backup))
			break ;
	}
	ret = ft_strnjoin(node->backup, NULL, read_n_find(fd, node->backup) + 1);
	node->backup = ft_strnjoin(node->backup + read_n_find(fd, node->backup) + 1, NULL, \
			ft_strlen(node->backup + read_n_find(fd, node->backup) + 1));
	if (!node->backup)
	{
		free_all(head, fd);
