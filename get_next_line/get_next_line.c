...
/*   Updated: 2021/12/05 06:26:23 by haryu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_newl(int fd, char *src)
{
	...
	while (1) 
	{
		get_byte = read(fd, src + total_byte, BUFFER_SIZE);
		//printf("total str : %s\n", src);
		if (get_byte == 0)
...
char	*get_next_line(int fd)
{
	...
	printf("str int : %s\n", total_read);
	index_e = get_next_newl(fd, total_read + index_s) + index_s;
	//printf("현재 end값 : %d \n",index_e);
	ret = ft_strndup(total_read + index_s, index_e - index_s + 1);
	if (!ret)
		return (NULL);
	return (ret);
}
