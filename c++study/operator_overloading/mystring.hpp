...
/*   Updated: 2022/09/30 11:17:24 by haryu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	explicit MyString(char c); // 문자 하나에서 생성
	MyString(const char *string); // 문자열에서 생성
	explicit MyString(int capacity); // 문자열 생성(정수)
...
MyString::MyString(int capacity) {
	...
	std::cout << "Capacity : " << MemCapacity << std::endl;
}
...
MyString&	MyString::insert(int loc, const MyString& str) {
	...
	char	*temp;
	int		i = 0;
	int		LengSum = StringSize + str.StringSize;
...
int MyString::compare(const MyString& str) const {
	for (int idx = 0; idx < std::min(StringSize, str.StringSize); idx++) {
		if (StringContent[idx] > str.StringContent[idx])
			return 1;
		else if (StringContent[idx] < str.StringContent[idx])
			return -1;
	}
